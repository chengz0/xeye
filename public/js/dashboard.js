...
dashboard.getSDA = function(){
     $.get("/proc/sda", function (data) {
        var ram_total = data.Data[0];
        var ram_used = Math.round((data.Data[1] / ram_total) * 100);
        var ram_free = Math.round((data.Data[2] / ram_total) * 100);

        $("#sda-total").text(ram_total);
        $("#sda-used").text(data.Data[1]);
        $("#sda-free").text(data.Data[2]);

        $("#sda-free-per").text(ram_free);
        $("#sda-used-per").text(ram_used);
    }, "json");
}
...
dashboard.getLastLog = function () {
    $.get("/proc/lastlogin", function (data) {
        var table = $("#lastlog_dashboard");
        var ex = document.getElementById("lastlog_dashboard");
        if ($.fn.DataTable.fnIsDataTable(ex)) {
            table.hide().dataTable().fnClearTable();
            table.dataTable().fnDestroy();
        }
